...
ARG TARGETARCH
RUN --mount=type=cache,target=/root/.cache/go-build bash -ec '\
  go mod tidy && \
  (cd cmd/collector && go mod tidy) && \
  mkdir -p bin && export GOARCH=$TARGETARCH && \
  make build-exporter build-collector build-btfhack'
