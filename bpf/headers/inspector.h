...
static __always_inline int skb_load_u8(struct sk_buff *skb, u32 off, u8 *out) {
  u32 len = BPF_CORE_READ(skb, len);
  if (off >= len)
    return -1;
  return bpf_probe_read_kernel(out, 1, (void *)BPF_CORE_READ(skb, head) + off);
}

static __always_inline int skb_load_bytes_bounded(struct sk_buff *skb, u32 off,
                                                  void *dst, u32 size) {
  u32 len = BPF_CORE_READ(skb, len);
  if (off > len || off + size > len)
    return -1;
  return bpf_probe_read_kernel(dst, size, (void *)BPF_CORE_READ(skb, head) + off);
}
