...
int kfree_skb(void *ctx) {
  struct sk_buff *skb;
  void *location;
  struct insp_pl_event_t event = {0};

  if (bpf_core_type_exists(struct trace_event_raw_kfree_skb)) {
    struct trace_event_raw_kfree_skb *args = ctx;
    skb = BPF_CORE_READ(args, skbaddr);
    location = BPF_CORE_READ(args, location);
  } else {
  ...
  if (enable_packetloss_stack){
      event.stack_id = bpf_get_stackid(ctx, &insp_pl_stack,
                                    KERN_STACKID_FLAGS);
  }

  bpf_perf_event_output(ctx, &insp_pl_event,
