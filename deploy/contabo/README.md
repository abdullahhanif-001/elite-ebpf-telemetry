# Contabo / bare-metal VPS deploy

## Quick install

```bash
sudo bash install.sh --channel contabo --metal
```

## Required layout

- `/opt/elite/bin/elite-agent` — agent binary
- `/opt/elite/config/config.yaml` — use `deploy/contabo/config.yaml` (no `kernellatency` on 6.8+)
