...
# Fresh VPS without PM2: light metrics soak only (keep-alive), not full chaos.
set -euo pipefail

LOG="/opt/elite/FINAL_STRESS_TEST.log"
mkdir -p "$(dirname "$LOG")"

if ! command -v pm2 >/dev/null 2>&1 || [[ ! -f /opt/elite/baseline/pm2-before.json ]]; then
  {
    echo "=== FINAL STRESS LIGHT $(date -Is) ==="
    echo "FINAL_STRESS_N/A_NO_PM2 — running metrics health soak only"
    systemctl is-active elite-agent
    curl -fsS --max-time 5 http://127.0.0.1:9102/metrics | grep -q elite_
    curl -fsS --max-time 5 http://127.0.0.1:9102/metrics | grep -q elite_predict_
    # light scrape loop (30s) — not chaos
    for _ in $(seq 1 6); do
      curl -fsS --max-time 3 -o /dev/null http://127.0.0.1:9102/metrics
      sleep 5
    done
    systemctl is-active elite-agent
    echo "FINAL_STRESS_LIGHT_PASS"
  } | tee -a "$LOG"
  exit 0
fi

BASELINE_RESTARTS=$(jq '[.[].pm2_env.restart_time] | add' /opt/elite/baseline/pm2-before.json)
TARGET_RESTARTS="${BASELINE_RESTARTS}"
