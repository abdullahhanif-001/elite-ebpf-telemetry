...
for unit in elite-updater.service elite-updater.timer; do
  src="${REPO_ROOT}/deploy/server/${unit}"
  [[ -f "${src}" ]] || die "missing ${src}"
  install -m 0644 "${src}" "/etc/systemd/system/${unit}"
  log "installed /etc/systemd/system/${unit}"
done

if [[ -f "${REPO_ROOT}/deploy/server/update.yaml" ]]; then
  mkdir -p "${ELITE_ROOT}/config"
  install -m 0644 "${REPO_ROOT}/deploy/server/update.yaml" "${ELITE_ROOT}/config/update.yaml"
