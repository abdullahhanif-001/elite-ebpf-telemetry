...
# Run ON VPS: bash /opt/elite/src/scripts/server/sched-ext-vps-prep.sh [phase]
...
phase_kernel_install() {
  ...
  log "after reboot run: bash ${ELITE_SRC:-/opt/elite/src}/scripts/server/sched-ext-vps-prep.sh verify"
}
...
phase_apply_patches() {
  ...
  if [[ -f "${CONTRIB}/selftests/rt_guard_stress.c" ]]; then
    ...
    if ! grep -q rt_guard_stress tools/testing/selftests/sched_ext/Makefile; then
      if [[ -f "${ELITE_SRC}/scripts/server/patch-sched-ext-makefile.py" ]]; then
        python3 "${ELITE_SRC}/scripts/server/patch-sched-ext-makefile.py"
      else
        python3 - <<'PY'
import pathlib
