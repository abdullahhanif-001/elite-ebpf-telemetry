#!/usr/bin/env bash
# Contabo world-proof orchestrator — PM2-safe. Do not restart PM2 apps.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${SCRIPT_DIR}/results/world-proof-${STAMP}"
mkdir -p "${OUT}"
export OUT SCRIPT_DIR REPO_ROOT STAMP
cd "${REPO_ROOT}"

bash "${REPO_ROOT}/deploy/contabo/pm2-guard.sh" | tee "${OUT}/pm2-before.txt"
...
bash "${REPO_ROOT}/deploy/contabo/pm2-guard.sh" | tee "${OUT}/pm2-after.txt"
...
add("Classic pains P1-P10", 20, 18, "WORLD_EBPF_COMPARISON + speed/overhead/audit")
...
add("Docs honesty (DECLINE rows)", 5, 5, "WORLD_EBPF_COMPARISON")
...
if score >= 90 and live_ok and phys_ok:
    ...
    verdict = "WORLD_BEST_PHYSICS_SPEED_SOFT"
elif score >= 85:
    verdict = "WORLD_BEST_PHYSICS_SPEED_SOFT"
else:
    verdict = "WORLD_BEST_PENDING"
if not live_ok and score >= 80:
    # Explicit provisional when predictive pts are bus-capped
    if live_sc <= 5:
        verdict = "WORLD_BEST_PROVISIONAL" if score < 90 else verdict
...
text = "\n".join(lines)
(script_dir / "WORLD_BEST_SCORECARD.md").write_text(text, encoding="utf-8")
(repo / "docs" / "WORLD_BEST_PROVIDER_SCORE.md").write_text(text, encoding="utf-8")
(out / "WORLD_BEST_SCORECARD.md").write_text(text, encoding="utf-8")
print(text)
print("SCORE", score, verdict)
PY

echo "WORLD_PROOF_DONE out=${OUT}"
