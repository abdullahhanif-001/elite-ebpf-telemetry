...
import (
	...
	"github.com/alibaba/kubeskoop/pkg/export"
...
func (i *inspServer) createListener(cfg *InspServerConfig) (net.Listener, error) {
	if cfg.Address == "" {
		if cfg.Port != 0 {
			...
			log.Warningf("port is deprecated, use address instead")
		} else {
			return nil, fmt.Errorf("listen address is empty")
		}
	}

	if err := security.ValidateListenAddress(cfg.Address); err != nil {
		return nil, err
	}
...
func (i *inspServer) start(cfg *InspServerConfig) error {
	...
	otelCtx, otelCancel := context.WithCancel(ctx)
	defer otelCancel()
	if cfg.Otel.Enabled {
		interval, _ := time.ParseDuration(cfg.Otel.Interval)
		bridge, err := export.NewBridge(export.Config{
			Enabled:   true,
			Endpoint:  cfg.Otel.Endpoint,
			Interval:  interval,
			BatchSize: cfg.Otel.BatchSize,
			Service:   cfg.Otel.Service,
		}, i.metricsServer.Registry())
		if err != nil {
			log.Warnf("OTel bridge disabled: %v", err)
		} else {
			go bridge.Run(otelCtx)
	...
	if cfg.Forecast.Enabled {
		...
		if fcfg.DecisionPath != "" {
			_ = os.MkdirAll(filepath.Dir(fcfg.DecisionPath), 0o755)
		}
		if fcfg.PolicyPath != "" {
			_ = os.MkdirAll(filepath.Dir(fcfg.PolicyPath), 0o755)
		}
		...
		if coll != nil {
			if err := i.metricsServer.Registry().Register(coll); err != nil {
				...
				// Still run so decision bus + snapshots advance; Collect may be absent if register failed hard.
				go runner.Run(forecastCtx)
			} else {
				log.Infof("forecaster metrics registered (elite_predict_*); starting runner")
				go runner.Run(forecastCtx)
			}
		}
	}
