...
Verification protocol — Abdullah Hanif. Start here. This directory contains **sanitized verdict lines** from live production server runs (`REAL_ONLY=1`, no mocks). Raw logs under `scripts/oneclick/results/` remain local and gitignored.

## Host requirements

| Host | sched_ext | Valid for SCX#1202 proof |
|------|-----------|---------------------------|
| production server (4 vCPU, kernel 6.19-rc7 + `CONFIG_SCHED_CLASS_EXT=y`) | YES | **Only valid proof host** |
...
Exit 0 = committed evidence bundle passes **strict** static + log cross-checks (`SCX1202_STRICT=1` default).

**Current bundle** [`VERIFICATION_20260903-012848`](./VERIFICATION_20260903-012848/) — `status=FULL` in MANIFEST. Strict verifier **PASS**.

Superseded PARTIAL bundles: `VERIFICATION_20260901-045612`, `VERIFICATION_20260902-063458` — do not cite as full proof.
...
bash benchmarks/ebpf-gates/scx1202-matrix-verify.sh       # → SCX1202_MATRIX_PASS=YES
...
| `02-SCX1202_MATRIX.verdict` or legacy `02-HOLY_GRAIL.verdict` | `SCX1202_MATRIX_PASS=YES fail=0 checks=12/12` |
...
## Documented limitations (LIMIT-SCX)

- **LIMIT-SCX-001:** Layer 3 `scx_rt_guard` reenqueues on RT `sched_switch`; 60s soak passes but large-scale performance not measured.
- **LIMIT-SCX-002:** Layer 2 watchdog skips `scx_partial_switch` mode; FAIR-only CPU hogging may still false-stall — partial fix.
