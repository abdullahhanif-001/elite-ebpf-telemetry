...
**World claim (scored):** Best **physics-speed production provider** on the public rubric in [OPS_PROVIDER_SCORE.md](OPS_PROVIDER_SCORE.md) — evidence-backed, not pitched.
...
|---|------|----------------|-----------|----------------|
| P1 | Sidecar tax (~500mCPU/pod Istio) | Density collapse | One agent/node | [COMPETITIVE_SPEED.md](../scripts/oneclick/COMPETITIVE_SPEED.md) S4 + Server CPU |
| P2 | K8s-only tooling | No bare VPS path | systemd Server first-class | Physics Pack / one-click (no CNI) |
...
|---------|-----|----------|------:|--------:|---------|-------|
| Elite agent + Physics Pack | Abdullah Hanif | Always-on physics + predict | 5 | 5 | **WIN (baseline)** | Server proven |
...
| Inspektor Gadget | Kinvolk/IG | Gadgets / optional metrics | 3 | 3 | PEER (optional) | Compose-only on server |
| Kepler | CNCF | Energy | 3 | 1 | OUT_OF_SCOPE | Energy axis not our product |
| Parca | Polar Signals | Continuous profiling | 2 | 1 | OUT_OF_SCOPE | Profiles optional compose |
...
| Grafana Beyla / OBI | Grafana | Zero-code app eBPF metrics | 3 | 2 | PEER idea / WIN physics default | Server default stays physics-light |

### Elite-only differentiator

| Axis | Elite | Market default | Verdict |
|------|-------|----------------|---------|
| VPS closed-loop Soft density | MOCK_ inject → `elite_predict_*` → Soft `cpu.max` | None of the above as Server systemd default | **PASS** |
...
Executive summary: [COMPETITIVE_PROOF.md](COMPETITIVE_PROOF.md). Rubric: [OPS_PROVIDER_SCORE.md](OPS_PROVIDER_SCORE.md).
