...
if [[ -x "${SCX_BIN_DIR:-/opt/scx/target/release}/scx_bpfland" ]]; then
  ...
  echo "LOADER=SKIP (scx_loader is DBus daemon — use scx_bpfland binary)"
else
  echo "LOADER=SKIP (scx_bpfland missing — sched_ext kernel not ready)"
fi
...
trap cleanup EXIT
if [[ -x /opt/scx/target/release/scx_bpfland ]]; then /opt/scx/target/release/scx_bpfland & LOADER_PID=$!; sleep 3; echo "LOADER=bpfland pid=${LOADER_PID}"; else echo "LOADER=SKIP"; fi
