#!/usr/bin/env bash
# scx1202-matrix-verify.sh — H1–H12 #1202 symptom-to-proof matrix.
set -euo pipefail
...
if [[ "${PASS}" -eq 12 ]]; then
  echo "SCX1202_MATRIX_PASS=YES fail=0 checks=12/12 host=${HOST}" | tee "${OUT_TXT}"
  our_goal_log "D2_holy_grail" "PASS" "${OUT_TXT}" "12/12"
  exit 0
fi
echo "SCX1202_MATRIX_PASS=NO pass=${PASS}/12 fail=${FAIL} host=${HOST}" | tee "${OUT_TXT}"
